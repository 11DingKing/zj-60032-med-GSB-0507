...
const fetchAppointment = async (silent = false) => {
  ...
  try {
    ...
    if (["IN_PROGRESS", "COMPLETED"].includes(appointment.value.status)) {
      relatedPromises.push(fetchMedicalRecord());
    }
    if (appointment.value.status === "COMPLETED" && isPatient.value) {
      relatedPromises.push(fetchReview());
    }
